...
| `Sandbox` | `DiffLines(ctx, c, baseSha) (int, error)` | | `git diff --numstat` inside the container. Advisory only: it runs where the agent owns `.git`, so the count is whatever the container says. The enforced budget legs are turns and wall clock, which the daemon counts itself |
| `Sandbox` | `CommitAndPush(ctx, ws, token, message) (headSha string, pushed bool, error)` | | Ends the agent phase first (removes the container, fails closed if it cannot confirm removal) before running any host git; `Tools`/`DiffLines` are invalid for that container once this has been called, though the closer `Tools` returned must still be closed (it reaps the `podman exec` child, and returns at once now the container is gone); token as a per-command header; `pushed` reports whether the push succeeded, not whether anything changed (the runner compares `headSha` to `Workspace.BaseSha` for that) |
| `Sandbox` | `ConflictMarkers(ctx, ws) (bool, error)` | | Host git scan of `<ws.BaseSha>..HEAD` for merge conflict markers the branch's own commits added. Valid only after `CommitAndPush`, which is what removes the container and commits the tree; the runner calls it before `OpenPullRequest` and records `Failed{Agent}` on a hit. Fails closed: a check that errors is `Failed{Infra}` and no pull request, since the branch is pushed either way |
