...
| `GET /api/status` | Operator | Bearer | Any operator | none | `{version, uptime_s, cases_by_state{state: n}, attempting[{attempt_id, repository, number, ordinal, elapsed_s}], queue_depth, concurrency, last_delivery_at}` | `unauthenticated` | Read | None. Read model over `cases`, `attempts` and `webhook_deliveries` |
