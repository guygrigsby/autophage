...
| `Sandbox` | `ConflictMarkers(ctx, ws) (bool, error)` | | Host git scan of `<ws.BaseSha>..HEAD` for merge conflict markers the branch's own commits added. Valid only after `CommitAndPush`, which is what removes the container and commits the tree; the runner calls it before `OpenPullRequest` and records `Failed{Agent}` on a hit |
